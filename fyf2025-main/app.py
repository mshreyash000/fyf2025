...
def get_dashboard_stats(date_filter='all'):
    # Base query
    query = Task.query
    ...
    if date_filter == 'today':
        ...
        query = query.filter(Task.task_date >= today - timedelta(days=7))
    ...
    # For tasks today, we need to filter again regardless of the main date filter
    tasks_today = len([t for t in filtered_tasks if t.task_date == today])
    
    completed_tasks = len([t for t in filtered_tasks if t.status == 'Completed'])
    total_revenue = sum(task.paid_amount for task in filtered_tasks)
    
    # For overdue tasks, we still use the time-based filter
    overdue_threshold = datetime.now() - timedelta(hours=24)
    overdue_tasks = len([t for t in filtered_tasks if 
                        t.status in ['Pending', 'In Progress', 'Hold'] and
                        t.created_at < overdue_threshold])
...
def dashboard_stats():
    try:
        ...
        stats = get_dashboard_stats(date_filter)
...
@app.route('/api/reports/staff-dues-summary')
@login_required
def get_staff_dues_summary():
    """Get Task Summary by Staff and Dues Amount Report
    
    This report provides comprehensive staff performance metrics including:
    - Staff Member Name
    - Total Tasks Assigned
    - Total Tasks Completed
    - Task Completion Rate (%)
    - Total Dues Amount (Sum of outstanding invoice balances)
    
    The query performs JOINs between User (Staff) and Tasks tables,
    aggregating task data and calculating dues for each staff member.
    ...
    SELECT 
        ...
        SUM(GREATEST(0, t.service_price - t.paid_amount)) AS total_dues
    FROM users u
    ...
    try:
        ...
        for staff in staff_users:
            # Build query for tasks assigned to this staff member
            query = Task.query.filter_by(assigned_to=staff.username)
            ...
            staff_tasks = query.all()
            
            # Calculate metrics
            total_tasks = len(staff_tasks)
            completed_tasks = sum(1 for t in staff_tasks if t.status == 'Completed')
            
            # Calculate completion rate
            completion_rate = round((completed_tasks / total_tasks * 100), 2) if total_tasks > 0 else 0
            
            # Calculate total dues amount (service_price - paid_amount for all tasks)
            total_dues = sum(
                max(0, (t.service_price or 0) - (t.paid_amount or 0)) 
                for t in staff_tasks
            )
            
            staff_summary.append({
                'staff_name': staff.username,
                'staff_email': staff.email or 'N/A',
                'total_tasks': total_tasks,
                'completed_tasks': completed_tasks,
        ...
        staff_summary.sort(key=lambda x: (x['completion_rate'], x['total_tasks']), reverse=True)
        
        return jsonify(staff_summary)
        
    except Exception as e:
        print(f"Error generating staff dues summary report: {e}")
        import traceback
...
def validate_task_row(row_data, row_num):
    ...
    if 'contact_number' in row_data and not pd.isna(row_data['contact_number']):
        contact_number = str(row_data['contact_number']).strip()
        # Remove all non-digit characters
        digits_only = ''.join(filter(str.isdigit, contact_number))
        # Check if it's exactly 10 digits
        if len(digits_only) != 10:
            errors.append(f"Row {row_num}: Contact number must be exactly 10 digits")
