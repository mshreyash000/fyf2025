...
const dateFilter = document.getElementById('dateFilter'); // Dashboard date filter
...
window.addEventListener('unhandledrejection', function(event) {
  ...
  // These are often from Chrome extensions or network issues
  console.log('Unhandled promise rejection (silent):', event.reason);
  event.preventDefault();
});

// Event Listeners
// Wrap all event listeners in try-catch to prevent unhandled promise rejections
...
userTypeSelectors.forEach(selector => {
  addSafeEventListener(selector, 'click', () => {
    userTypeSelectors.forEach(s => s.classList.remove('active'));
    selector.classList.add('active');
  });
});

addSafeEventListener(loginForm, 'submit', handleLogin);
addSafeEventListener(logoutBtn, 'click', handleLogout);

// Sidebar Toggle Event Listeners
if (sidebarToggle) {
  addSafeEventListener(sidebarToggle, 'click', closeSidebar);
}

if (mobileSidebarToggle) {
  addSafeEventListener(mobileSidebarToggle, 'click', openSidebar);
}

if (sidebarOverlay) {
  addSafeEventListener(sidebarOverlay, 'click', closeSidebar);
}

// Close sidebar when clicking nav links on mobile
navLinks.forEach(link => {
  addSafeEventListener(link, 'click', (e) => {
    e.preventDefault();
...
addSafeEventListener(addServiceBtn, 'click', showServiceForm);
addSafeEventListener(cancelServiceBtn, 'click', hideServiceForm);
addSafeEventListener(serviceForm, 'submit', handleServiceSubmit);
addSafeEventListener(serviceSearch, 'input', filterServices);
...
// Dashboard Date Filter Event Listener
if (dateFilter) {
  dateFilter.addEventListener('change', updateDashboard);
}

// Payment Modal Event Listeners
addSafeEventListener(closePaymentModal, 'click', () => {
  paymentModal.style.display = 'none';
});

addSafeEventListener(cancelPaymentBtn, 'click', () => {
  paymentModal.style.display = 'none';
});

addSafeEventListener(confirmPayment, 'click', completeTaskWithPayment);

// User Management Event Listeners
addSafeEventListener(closeUserModal, 'click', () => {
  userManagementModal.style.display = 'none';
});

addSafeEventListener(cancelUserBtn, 'click', () => {
  userManagementModal.style.display = 'none';
});

addSafeEventListener(saveUserBtn, 'click', saveUser);
...
// Contact Number Input Masking with +91 prefix
...
if (contactNumberInput) {
  ...
  contactNumberInput.addEventListener('input', function(e) {
    ...
    // Always ensure +91 prefix exists
    if (!value.startsWith('+91')) {
      this.value = '+91 ';
      return;
    }
    
    // Remove all non-digit characters after +91
    const prefix = '+91 ';
  ...
  contactNumberInput.addEventListener('keydown', function(e) {
    ...
    if (cursorPosition > 4 && e.key.match(/[^0-9]/) && 
...
function initSocketIO() {
    // Only initialize WebSocket for authenticated users
    if (!currentUser) {
        console.log('Skipping WebSocket initialization - no authenticated user');
        return;
    }
    ...
    try {
        ...
        socket.on('connect_error', (error) => {
            ...
            // Don't show error to user for connection issues - fallback to polling
        });
        ...
        // Continue without real-time updates - app will still work with polling
    }
}
...
async function apiFetch(url, options = {}) {
  ...
  try {
    ...
    if (res.status === 403) {
      console.log(`Access denied to ${url} - this may be expected for your user role`);
      // Don't show error toast for 403 - handle silently
    ...
    // Only log and show errors for non-403 status codes
...
function populateStaffDropdown() {
  ...
  if (currentUser.role === 'staff') {
    // Staff users don't have access to user list, use fallback
    handleUserListFallback();
  } else {
    apiFetch('/api/users')
      .then(users => {
      ...
      .catch(() => {
        // If not permitted, fallback to existing in-memory list
        handleUserListFallback();
      });
  }
}
...
// Handle user list fallback for staff users
function handleUserListFallback() {
  ...
  if (currentUser && currentUser.role === 'staff') {
    ...
    // For managers/admins, use existing in-memory list if available
...
function populateStaffList() {
  ...
  // Only admins and managers can view the full staff list
  if (currentUser.role === 'staff') {
    // Staff users get a simplified view or message
    staffListContainer.innerHTML = '<p class="text-center">Staff list is only available to managers and admins.</p>';
    return;
  }
...
function showServiceForm() {
  ...
  serviceFormContainer.scrollIntoView({ behavior: 'smooth' });
}
...
function handleTaskSubmit(e) {
  ...
  // CRITICAL VALIDATION: Prevent task generation when 'All Branches' is selected
  const branchCodeValue = document.getElementById('branchCode').value;
  
  if (!branchCodeValue || branchCodeValue === '' || branchCodeValue === 'all') {
    showToast('Please select a specific branch. Tasks cannot be created for "All Branches".', 'error');
    return; // Block task generation
  }
...
// Validate contact number (must be exactly 10 digits after +91 prefix)
function validateContactNumber(contactNumber) {
  // Remove all non-digit characters and the +91 prefix
...
function validatePayment(taskData) {
  ...
  if (!validateContactNumber(taskData.contactNumber)) {
    showToast('Contact number must be exactly 10 digits. Please check and try again.', 'error');
    return false;
  }
...
function saveTask() {
  ...
  // Double-check branch validation before saving
  if (!branchCodeValue || branchCodeValue === '' || branchCodeValue === 'all') {
    showToast('Invalid branch selection. Please select a specific branch.', 'error');
    return;
  }
...
function updateEnhancedReports() {
  ...
  // Update new staff dues report
  updateStaffDuesReport(reportDateFilter);
}
...
// Update Staff Dues Report (NEW REPORT)
function updateStaffDuesReport(dateFilter = 'all') {
  const staffDuesReportTable = document.getElementById('staffDuesReportTable');
  if (!staffDuesReportTable) return;
  
  // Show loading state
  staffDuesReportTable.innerHTML = '<tr><td colspan="5" style="text-align: center;">Loading report...</td></tr>';
  
  // Fetch staff dues summary from backend
  apiFetch(`/api/reports/staff-dues-summary?date=${dateFilter}`)
    .then(data => {
      staffDuesReportTable.innerHTML = '';
      
      if (!data || data.length === 0) {
        staffDuesReportTable.innerHTML = '<tr><td colspan="5" style="text-align: center;">No data available</td></tr>';
        return;
      }
      ...
      data.forEach(staff => {
        const row = document.createElement('tr');
        row.innerHTML = `
          <td><strong>${staff.staff_name}</strong><br><small>${staff.staff_email}</small></td>
          <td>${staff.total_tasks}</td>
          <td>${staff.completed_tasks}</td>
          <td><span class="completion-rate" style="font-weight: bold; color: ${getCompletionRateColor(staff.completion_rate)}">${staff.completion_rate}%</span></td>
          <td><strong>₹${staff.total_dues.toLocaleString()}</strong></td>
        `;
        staffDuesReportTable.appendChild(row);
      });
    })
    .catch(error => {
      console.error('Error fetching staff dues report:', error);
      staffDuesReportTable.innerHTML = '<tr><td colspan="5" style="text-align: center; color: red;">Error loading report. Please try again.</td></tr>';
    });
}

// Helper function to get color based on completion rate
function getCompletionRateColor(rate) {
  if (rate >= 80) return '#22c55e'; // Green for high completion
  if (rate >= 50) return '#f59e0b'; // Orange for medium completion
  return '#ef4444'; // Red for low completion
}

// Update dashboard statistics from backend
function updateDashboard() {
  // Get the date filter value if available
  const dateFilterValue = dateFilter ? dateFilter.value : 'all';
  
  apiFetch(`/api/dashboard/stats?date_filter=${dateFilterValue}`)
    .then(stats => {
      if (document.getElementById('totalTasks')) {
        document.getElementById('totalTasks').textContent = stats.total_tasks;
      }
